...
test("@claim:accessible-interactions pass Axe, focus home, expose command scrolling, targets, and accurate busy state", async () => {
  ...
  await page.setViewportSize({ width: 390, height: 844 }); const copyButton = page.getByRole("button", { name: "Copy install command" }); const copySize = await copyButton.evaluate((button) => ({ clientWidth: button.clientWidth, scrollWidth: button.scrollWidth, clientHeight: button.clientHeight })); assert.ok(copySize.clientWidth >= 44 && copySize.clientHeight >= 44); assert.ok(copySize.scrollWidth <= copySize.clientWidth, "mobile copy label must not be clipped");
