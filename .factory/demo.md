...
- Browser demo: open `/demo` or `/?demo=1`. It immediately shows the completed `checkout-v2` sample result. The banner says that sample data stays in this tab; **Reset demo** restores the bundled inputs and **Start for real** discards the `demo:flag-removal-map` session key before returning home.
