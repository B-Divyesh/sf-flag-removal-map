...
test("@claim:accessible-interactions pass Axe, focus home, expose command scrolling, targets, and accurate busy state", async () => {
  ...
  await demoLink.click(); await page.getByRole("heading", { name: "Classify a sample flag" }).waitFor(); await page.getByRole("link", { name: "Flag Removal Map home" }).click(); await page.getByRole("heading", { name: "Review completed flags before removal" }).waitFor(); await page.waitForFunction(() => document.activeElement?.tagName === "H1");
