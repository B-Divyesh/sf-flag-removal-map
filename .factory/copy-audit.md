...
| 14 | An abstract topographic repository map showing three evidence markers joined by a red route. | Pass |
...
| 16 | Terminal output from flag-removal-map demo showing one removal candidate, three references, and a temporary plan path. | Pass |
...
The current catalog line is 10 words, verb-first, and 72 characters: “Review completed feature flags before removal with dated local evidence.”

| Concept | One term |
| --- | --- |
| Provider input | provider export |
| Usage input | dated usage report |
| Count field | evaluation count |
| Code matches | references |
| Output | removal plan |
| Browser action | classify |
| CLI action | analyze |
| Candidate state | Removal candidate |
